...
const getReactDOM = () =>
  (Spicetify && Spicetify.ReactDOM) || window.ReactDOM || reactDOM;
...
class LyricsContainer extends react.Component {
  ...
  async fetchVideoBackgroundWithLyrics(track, lyrics = []) {
    ...
    if (
      this.state.videoBackground &&
      this.state.videoBackground.uri !== info.uri
    ) {
      ...
      react.useEffect(() => {
        ...
        const fetchResults = async () => {
          ...
          try {
            ...
            const results = await VideoManager.searchMultipleVideos(
              query,
              info.uri,
              info,
            );
      ...
      return react.createElement(
        ...
        react.createElement(
          ...
          react.createElement(
            ...
            { label: getText("tooltips.searchYoutube") || "Search on YouTube" },
            react.createElement(
              "button",
              {
              ...
              "button",
              {
                ...
                onClick: async () => {
                  try {
                    ...
                    if (text) {
                      ...
                      if (match) {
                        ...
                        Spicetify.showNotification(
                          `✓ ${getText("notifications.videoSynced").replace("{videoId}", match[1]).replace("{offset}", offset)}`,
                        );
                      } else if (
                        trimmed.length === 11 &&
                        ...
                        Spicetify.showNotification(
                          "✓ " +
                            getText("notifications.videoSet", {
                              videoId: trimmed,
                            }),
                        );
                      } else {
                        Spicetify.showNotification(
                          getText("notifications.invalidClipboardUrl"),
                          true,
                          2000,
                        );
                      }
                    ...
                    Spicetify.showNotification(
                      getText("notifications.failedReadClipboard"),
                      true,
                      2000,
                    );
                  }
            ...
            "button",
            {
              onClick: () => {
                if (!videoId || videoId.length !== 11) {
                  Spicetify.showNotification(
                    getText("notifications.invalidId"),
                    true,
                    2000,
                  );
                  return;
                ...
                const result = VideoManager.setManualVideo(
                  info,
                  videoId,
                  offset,
                );
                if (result) {
                  ...
                  Spicetify.showNotification(
                    "✓ " + getText("notifications.videoSetSaved", { videoId }),
                    false,
                    2000,
                  );
                }
          ...
          react.createElement(
            "button",
            {
              onClick: () => {
                ...
                Spicetify.PopupModal.hide();
                Spicetify.showNotification(
                  getText("notifications.videoReset"),
                  false,
                  2000,
                );
  ...
  async fetchLyrics(track, mode = -1, refresh = false) {
      ...
      if (hasContent) {
        if (hasSynced) {
          ...
          if (upgradeAttempted) {
            ...
            const enabledSyncProviders = syncCapableProviders.filter(
              (p) => CONFIG.providers[p]?.on,
            );
    ...
    if (tempState?.copyright && tempState?.provider) {
      ...
      const duplicatePattern = new RegExp(
        `(synced|unsynced)?\\s*lyrics provided by ${providerName}`,
        "i",
      );
    ...
    if (mode === -1) {
      if (
        ...
        this.state.explicitMode !== KARAOKE
      ) {
        finalMode = this.state.explicitMode;
      } else if (
        this.state.lockMode !== -1 &&
        this.state.lockMode !== KARAOKE
      ) {
    ...
    if (tempState.uri !== this.state.uri || refresh) {
      ...
      if (tempState.synced) {
        ...
        const geniusText = tempState.genius
          .replace(/<br>/g, "\n")
          .replace(/<[^>]*>/g, "");
      ...
      const initialCurrentLyrics =
        this.state.currentLyrics?.length > 0 &&
        this.state.currentLyrics?.some((l) => l.text || l.text2)
      ...
      this.setState({
        ...
        ...(tempState.musixmatchTranslation && {
          musixmatchTranslation: tempState.musixmatchTranslation,
        }),
        ...(tempState.neteaseTranslation && {
          neteaseTranslation: tempState.neteaseTranslation,
        }),
  ...
  async lyricsSource(lyricsState, mode) {
    ...
    if (
      ...
      this._setCurrentLyrics(
        Array.isArray(optimizedLyrics) ? optimizedLyrics : [],
      );
      return;
    }
    ...
    const updateCombinedLyrics = (force = false) => {
      ...
      this._setCurrentLyrics(
        Array.isArray(optimizedTranslations) ? optimizedTranslations : [],
      );
    };
    ...
    this._setCurrentLyrics(
      Array.isArray(optimizedOriginal) ? optimizedOriginal : [],
    );
    ...
    const isMode1Phonetic =
      String(displayMode1).includes("romaji") ||
      String(displayMode1).includes("pinyin") ||
      String(displayMode1).includes("romaja");
    const isMode2Phonetic =
      String(displayMode2).includes("romaji") ||
      String(displayMode2).includes("pinyin") ||
      String(displayMode2).includes("romaja");
    ...
    if (isMode2Phonetic && !isMode1Phonetic) {
      firstMode = displayMode2;
      secondMode = displayMode1;
      firstModeKey = "mode2";
      secondModeKey = "mode1";
    } else if (isMode1Phonetic && !isMode2Phonetic) {
      firstMode = displayMode1;
      secondMode = displayMode2;
      firstModeKey = "mode1";
      secondModeKey = "mode2";
    } else {
    ...
    const promise1 = processMode(firstMode, lyrics)
      .then((result) => {
        if (this.state.uri !== uri) return;
        if (this._dmResults?.[currentUri])
          this._dmResults[currentUri][firstModeKey] = result;
        updateCombinedLyrics(true);
      })
      .catch((error) => {
        if (this.state.uri !== uri) return;
    ...
    const promise2 = new Promise((resolve) => setTimeout(resolve, 500)).then(
      () => {
        return processMode(secondMode, lyrics)
          .then((result) => {
            if (this.state.uri !== uri) return;
            if (this._dmResults?.[currentUri])
              this._dmResults[currentUri][secondModeKey] = result;
            updateCombinedLyrics(true);
          })
          .catch((error) => {
            if (this.state.uri !== uri) return;
    ...
    Promise.allSettled([promise1, promise2]).then(() => {
      ...
      const originalCount = Array.isArray(originalLyrics)
      ...
      if (originalCount > 0 && translatedCount !== originalCount) {
        console.warn(
          `[Lyrics+] Skip cache - line count mismatch: original=${originalCount}, translated=${translatedCount}`,
        );
        return;
      }
  ...
  optimizeTranslations(originalLyrics, mode1, mode2) {
    ...
    const optimized = TranslationUtils.optimizeTranslations(
      originalLyrics,
      mode1,
      mode2,
    );
  ...
  // index alignment when counts match. Returns null if it can't confidently cover
  // most lines, so the caller uses computed romaji instead of a bad partial map.
  _alignProviderRomanization(baseLyrics, romaLines) {
    if (!Array.isArray(baseLyrics) || !baseLyrics.length) return null;
    if (!Array.isArray(romaLines) || !romaLines.length) return null;

    const baseTimed = baseLyrics.every((l) => typeof l.startTime === "number");
    const romaTimed = romaLines.every((l) => typeof l.startTime === "number");
    let matched = 0;
    let result = null;

    if (baseTimed && romaTimed) {
      const byTime = new Map();
      for (const l of romaLines) byTime.set(l.startTime, l.text);
      result = baseLyrics.map((line) => {
        let text = byTime.get(line.startTime);
        if (text === undefined) {
          let best = null;
          let bestD = 400; // ms tolerance for jitter between the two tracks
          for (const l of romaLines) {
            const d = Math.abs(l.startTime - line.startTime);
            if (d < bestD) {
              bestD = d;
              best = l;
            }
          }
          text = best ? best.text : undefined;
        }
        if (text !== undefined && text !== "") matched++;
        return { ...line, text: text !== undefined ? text : line.text };
      });
    } else if (romaLines.length === baseLyrics.length) {
      result = baseLyrics.map((line, i) => {
        const text = romaLines[i] && romaLines[i].text;
        if (text) matched++;
    ...
    // Need most lines covered, else the computed pipeline does a better job.
    if (matched < Math.ceil(baseLyrics.length * 0.6)) return null;
    return result;
  }
  ...
  async getTraditionalConversion(lyricsState, lyrics, language, displayMode) {
    ...
    // Prefer NetEase's official romanized lyrics (romalrc) over computed romaji for
    // Japanese — it's human-made and accurate. Align it to the current lines by
    // timestamp (fallback to index) and return directly, skipping kuromoji. Only
    // when coverage is good; otherwise fall through to computed romanization.
    if (
      language === "ja" &&
      displayMode === "romaji" &&
      Array.isArray(lyricsState.neteaseRomanized) &&
      lyricsState.neteaseRomanized.length
    ) {
      const aligned = this._alignProviderRomanization(
        lyrics,
        lyricsState.neteaseRomanized,
      );
      if (aligned) return aligned;
    }

    // Bump this version whenever the romanization pipeline changes (Translator
    // logic, OpenCC/pinyin passes, etc.) so previously-cached romaji generated by
    // the old code is treated as a miss and regenerated — the cache otherwise
    // only invalidates on line-count change and would serve stale conversions.
    const TRAD_CACHE_VERSION = "v7";
    const cacheKey = `${lyricsState.uri}:trad:${TRAD_CACHE_VERSION}:${language}:${displayMode}`;
        ...
        pendingTimer = setTimeout(() => {
          try {
            ...
            if (window.lyricsPlusDebug)
              console.warn("[Lyrics+] Could not show notification:", e);
          }
        }, 3000);

        const translated = await this.translateLyrics(
          language,
          lyrics,
          displayMode,
        );
    ...
    try {
      if (language === "ja") {
        ...
        if (targetConvert === "pinyin") {
          ...
          if (!anyChanged) {
            Spicetify.showNotification(
              getText("notifications.pinyinLibraryUnavailable"),
              true,
              4000,
            );
          }
          ...
          if (targetConvert === "cn") {
            Spicetify.showNotification(
              getText("notifications.conversionSkippedAlreadySimplified"),
              false,
              2000,
            );
            return lyrics;
      ...
      Spicetify.showNotification(
        getText("notifications.conversionFailed", {
  ...
  async saveLocalLyrics(uri, lyrics) {
    ...
    try {
      ...
      const cachedUris = JSON.parse(
        localStorage.getItem(`${APP_NAME}:cached-uris`) || "[]",
      );
      if (!cachedUris.includes(uri)) {
        ...
        localStorage.setItem(
          `${APP_NAME}:cached-uris`,
          JSON.stringify(cachedUris),
        );
      }
  ...
  processLyricsFromFile(event) {
    ...
    if (file[0].size > 1024 * 1024) {
      Spicetify.showNotification(
        getText("notifications.fileTooLarge"),
        true,
        3000,
      );
      return;
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!parsedKeys.length) {
          Spicetify.showNotification(
            getText("notifications.noLyricsInFile"),
            true,
            3000,
          );
          return;
        ...
        const newCurrentLyrics =
          localLyrics.synced || localLyrics.unsynced || [];
        ...
        Spicetify.showNotification(
          "✓ " +
            getText("notifications.loadedLyricsFromFile", {
              types: parsedKeys.join(", "),
            }),
          false,
          3000,
        );
        ...
        console.error(e);
        Spicetify.showNotification(
          getText("notifications.failedLoadLyricsInvalidFormat"),
          true,
          3000,
        );
      }
    ...
    reader.onerror = (e) => {
      console.error(e);
      Spicetify.showNotification(
        getText("notifications.failedReadFileCorrupted"),
        true,
        3000,
      );
    };
  ...
  componentDidMount() {
    ...
    window.lyricsPlusDebug =
      localStorage.getItem("lyrics-plus:debug") === "true";
    ...
    this.onFontSizeChange = (event) => {
      const mode =
        this.state.mode !== -1 ? this.state.mode : this.getCurrentMode();
      const isCompactSyncedMode =
        mode === SYNCED && !!CONFIG.visual["synced-compact"];
      ...
      if (isEnabled) {
        ...
        if (document.fullscreenElement) {
          document
            .exitFullscreen()
            .catch((e) =>
              console.warn("[Lyrics+] Failed to exit fullscreen:", e),
            );
        }
  ...
  updateVisualOnConfigChange() {
    ...
    if (CONFIG.visual["transparent-background"]) {
      ...
      this.styleVariables = {
        ...
        "--lyrics-color-background":
          this.state.colors.background || CONFIG.visual["background-color"],
        "--lyrics-highlight-background":
          this.state.colors.inactive || CONFIG.visual["highlight-color"],
        "--lyrics-background-noise": CONFIG.visual.noise
          ? "var(--background-noise)"
          : "unset",
        "--lyrics-background-brightness": `brightness(${brightness})`,
    ...
    this.styleVariables = {
      ...
      "--lp-ui-switch-on":
        CONFIG.visual["ui-switch-on-color"] || "var(--spice-button)",
      "--lp-ui-switch-off":
        CONFIG.visual["ui-switch-off-color"] ||
        "rgba(var(--spice-rgb-subtext), 0.2)",
      "--lp-ui-btn-bg":
        CONFIG.visual["ui-button-bg-color"] ||
        "var(--spice-button-disabled, rgba(255,255,255,0.08))",
      "--lp-ui-btn-text":
        CONFIG.visual["ui-button-text-color"] || "var(--spice-text)",
      "--lp-fab-bg":
        CONFIG.visual["ui-fab-bg-color"] ||
        "rgba(var(--spice-rgb-main, 20, 20, 20), 0.72)",
      "--lp-fab-icon":
        CONFIG.visual["ui-fab-icon-color"] || "var(--spice-button)",
      "--lp-ui-accent":
        CONFIG.visual["ui-accent-color"] || "var(--spice-button)",
    };
  ...
  render() {
    ...
    const translationStatusForTrack =
      ...
      (!this.state.translationStatus.trackUri ||
    ...
    const brightnessVal =
      brightness !== undefined && brightness !== null ? brightness : 100;
    ...
    if (this.state.isFADMode) {
      ...
      const bgColor =
        ...
        CONFIG.visual["background-color"] ||
        "#121212";
      const highlightColor =
        this.state.colors?.inactive ||
        CONFIG.visual["highlight-color"] ||
    ...
    const originalLanguage = originalLyrics
      ? Utils.detectLanguage(originalLyrics)
      : null;
    ...
    let romanizeInfo = null;
    if (originalLanguage === "ja")
      romanizeInfo = { modeKey: "japanese", value: "romaji", label: "Romaji" };
    else if (originalLanguage === "ko")
      romanizeInfo = { modeKey: "korean", value: "romaja", label: "Romaja" };
    else if (originalLanguage && originalLanguage.startsWith("zh"))
      romanizeInfo = { modeKey: "chinese", value: "pinyin", label: "Pinyin" };
    const romanizeActive =
      !!romanizeInfo &&
      CONFIG.visual[`translation-mode:${romanizeInfo.modeKey}`] ===
        romanizeInfo.value;
    ...
    showTranslationButton =
      potentialMode === SYNCED || potentialMode === UNSYNCED || mode === -1;
    ...
    const out = react.createElement(
      ...
      react.createElement(
        ...
        react.createElement(AdjustmentsMenu, {
          mode,
          currentLang: CONFIG.visual["ui-language"],
        }),
        ...
        react.createElement(
          ...
          react.createElement(
            "button",
            {
              ...
              style: {
                color: "var(--lp-fab-icon, var(--spice-button))",
                fontWeight: "bold",
                fontSize: "16px",
          ...
          react.createElement(
            ...
            react.createElement(
              ...
              react.createElement("svg", {
                ...
                dangerouslySetInnerHTML: {
                  __html:
                    '<path d="M14.5 13.5h-13A.5.5 0 011 13V3a.5.5 0 01.5-.5h13a.5.5 0 01.5.5v10a.5.5 0 01-.5.5zM2 12h12V4H2v8z"/><path d="M6 6l4 2-4 2V6z"/>',
                },
              }),
            ),
          ),
        react.createElement(
          Spicetify.ReactComponent.TooltipWrapper,
          {
            label: this.state.isCached
              ? getText("tooltips.lyricsCached")
              : getText("tooltips.cacheLyrics"),
          },
          react.createElement(
            "button",
            {
              ...
              onClick: () => {
                ...
                if (!synced && !unsynced && !karaoke && !genius) {
                  Spicetify.showNotification(
                    getText("notifications.noLyricsCache"),
                    true,
                    2000,
                  );
                  return;
                ...
                if (this.state.isCached) {
                  ...
                  Spicetify.showNotification(
                    `✓ ${getText("notifications.cacheSuccess")}`,
                    false,
                    2000,
                  );
                }
        ...
        romanizeInfo &&
          react.createElement(
            "div",
            { className: "lyrics-romanize-wrap" },
            react.createElement(
              Spicetify.ReactComponent.TooltipWrapper,
              {
                label: romanizeActive
                  ? "Show original"
                  : `Romanize (${romanizeInfo.label})`,
              },
              react.createElement(
                "button",
                {
                  className: "lyrics-config-button",
                  style: {
                    color: romanizeActive
                      ? "var(--spice-button)"
                      : "var(--lp-fab-icon, var(--spice-button))",
                    fontWeight: "bold",
                    fontSize: "15px",
                  },
                  onClick: () => {
                    const next = romanizeActive ? "none" : romanizeInfo.value;
                    CONFIG.visual[`translation-mode:${romanizeInfo.modeKey}`] =
                      next;
                    ConfigUtils.setPersisted(
                      `lyrics-plus:visual:translation-mode:${romanizeInfo.modeKey}`,
                      next,
                    );
                    lyricContainerUpdate();
                  },
    ...
    if (this.state.isFullscreen)
      return createPortalSafe(out, this.fullscreenContainer);
