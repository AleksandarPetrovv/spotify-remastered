...
async function scenario(failSecond, cancelAfterFirst) {
  const order = [];
  let index = -1;
  const env = {collection:{id:'collection',entries:[1,2,3].map(n=>({url:'song'+n,title:'song'+n}))}, importing:false, cancelled:false,
...
async function nested(cycle) {
  const added=[];
  const env={collection:{id:'root',entries:[{url:'album',title:'album'},{url:'tail',title:'tail'}]},importing:false,cancelled:false,bulkProgress:null,lastSavedId:null,job:null,status:{},primary:{},another:{},uri:'playlist',name:'test',renderBulk(){},stopped(){return false;},background:null,
